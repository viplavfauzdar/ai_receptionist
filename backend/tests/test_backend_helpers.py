...
def test_ensure_sqlite_compatibility_adds_missing_columns(monkeypatch: pytest.MonkeyPatch):
    ...
    class FakeInspector:
        def get_table_names(self):
            return ["call_logs", "appointment_requests"]
...
def test_main_request_and_session_helpers(db_session):
    with db_session() as db:
        ...
        business = models_module.Business(name="Acme", twilio_number="+1 (678) 462-4453", greeting="Hello")
        db.add(business)
        db.commit()
        db.refresh(business)
    ...
    assert main_module._should_create_request("BOOK_APPOINTMENT", "BOOKING_COMPLETE", {}) is True
    assert main_module._should_create_request("CALLBACK_REQUEST", "CALLBACK_READY", {}) is True
